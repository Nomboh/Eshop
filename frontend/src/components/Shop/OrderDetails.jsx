...
function OrderDetails() {
	...
	const refundOrderUpdateHandler = async () => {
		await axios
			...
			.then((res) => {
				toast.success("Order refund updated successfully")
				dispatch(getAllShopOrders(seller._id))
			})
			.catch((error) => {
				toast.error(error.response.data.message)
		...
		<div className={`py-4 min-h-screen ${styles.section}`}>
			<div className="w-full flex items-center justify-between">
				<div className="flex items-center">
					...
					<h1 className=" pl-3 text-[25px]">Order Details</h1>
				</div>
				<Link to={"/dashboard-orders"}>
					<div
						className={`${styles.button} !bg-[#fce1e5] !rounded-[4px] !h-[45px] font-[600] text-[18px] text-[#e94560]`}>
						Order List
					</div>
				</Link>
			</div>
